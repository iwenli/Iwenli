...
using Iwenli.CodeGenerate.Properties;
	...
	public class JsonClassGenerator : IJsonClassGeneratorConfig
	{
		...
		public void GenerateClasses()
		{
			...
			if (writeToDisk)
			{
				...
				if (writeToDisk && !NoHelperClass && ExplicitDeserialization) File.WriteAllBytes(Path.Combine(TargetFolder, "JsonClassHelper.cs"), Resources.JsonClassHelper);
				if (SingleFile)
				{
		...
		public static readonly string[] FileHeader = new[] {
			...
			$"Copyright (C) {DateTime.Now.Year} 天下商机（txooo.com）版权所有"
		};
	}
}
