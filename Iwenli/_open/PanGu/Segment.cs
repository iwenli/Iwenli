    ...
    //@"[０-９\d]+|[^ａ-ｚＡ-Ｚa-zA-Z0-9０-９\u4e00-\u9fa5]|[ａ-ｚＡ-Ｚa-zA-Z]+|[\u4e00-\u9fa5]+";
        ...
        public static void Init(string fileName)
        {
            lock (_LockObj)
            {
                ...
                if (fileName == null)
                {
                    LogHelper.GetLogger("PanGu").Info("默认在运行目录下加载数据：" + AppDomain.CurrentDomain.BaseDirectory);
                    string _config = AppDomain.CurrentDomain.BaseDirectory + "PanGu\\PanGu.xml";
                    if (!System.IO.File.Exists(_config))
                    {
                        _config = AppDomain.CurrentDomain.BaseDirectory + "App_Data\\PanGu\\PanGu.xml";
                        if (!System.IO.File.Exists(_config))
                        {                            
                        ...
                        {
                            Setting.SettingLoader loader = new PanGu.Setting.SettingLoader(_config);
                        }
                    }
                    else
                    {
