...
namespace Iwenli.Extension
{
	...
	public static class ImageExtension
	{
		...
		public static byte[] CompressForByte(this byte[] img)
		{
			int _com = 0;
			if ((img.Length / 1048576.0) > 1)
			{//文件太大
				_com++;
			}
			using (MemoryStream ms1 = new MemoryStream(img))
			{
				using (var _img = Image.FromStream(ms1))
				{
					double _width = _img.Width, _height = _img.Height;
					double _wh = 1080.0 / _img.Height;
					_wh = _wh < 1 ? _wh : 1920.0 / _img.Width;
					if (_wh < 1)
					{//尺寸太大
						_com++;
						_width = _img.Width * _wh;
						_height = _img.Height * _wh;
					}
					if (_com > 0)
					{
						return Compress(_img, new Size((int)_width, (int)_height));
					}
				}
			}
			return img;
		...
		/// <returns></returns>
		public static byte[] Compress(this Image img, Size newSize)
		{
			using (Bitmap outBmp = new Bitmap(newSize.Width, newSize.Height))
			{
				using (Graphics g = Graphics.FromImage(outBmp))
				{
					...
					outBmp.Save(_stream, System.Drawing.Imaging.ImageFormat.Jpeg);
