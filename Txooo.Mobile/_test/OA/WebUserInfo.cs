...
namespace Txooo.Mobile.OA
{
    ...
    public class WebUserInfo : Txooo.Data.Entity.DataRowEntityBase
    {
        #region 属性
        ...
        public static WebUserInfo GetUserInfoByUserId(PlatformType platform, string userId)
        {
            OAUserInfo _userInfo = new OAUserInfo();

            string _sql = "SELECT * FROM [View_Mobile_UserInfo] WHERE [weixin_sn]='" + userId + "'";
            List<WebUserInfo> _list = DataEntityHelper.GetDataRowEntityList<WebUserInfo>("TxoooOffice", _sql, "");
            if (_list.Count == 1)
            {
                return _list[0];
            }
        ...
        public static WebUserInfo GetUserInfoByUsername(PlatformType platform, string username)
        {
            OAUserInfo _userInfo = new OAUserInfo();
        ...
        public static WebUserInfo GetUserInfoByUsername(PlatformType platform, long userid)
        {
            OAUserInfo _userInfo = new OAUserInfo();
        ...
        public static WebUserInfo GetUserInfoByMobile(PlatformType platform, string Mobile)
        {
            OAUserInfo _userInfo = new OAUserInfo();
        ...
        public static bool OAUserBinding(string userid, string mobile)
        {
            using (Txooo.TxDataHelper helper = Txooo.TxDataHelper.GetDataHelper("TxoooOffice"))
            {
                string _sql = "UPDATE [TxoooOffice].[dbo].[Tx_Employee] SET [weixin_sn]='" + userid + "' WHERE [IsDimissory]=0 And [IsClocked]=0 AND [Mobile]='" + mobile + "'";
