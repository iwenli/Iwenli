...
//弹出层
//title	弹出层的标题	必填，纯文本
//ontent	弹出层的内容	text:文本内容/id:页面里某id的子标签/img:图片/url:get或post某一页面里的html，该页面要求只包含body的子标签/iframe：目标地址在框架显示
//width	弹出层的宽	必填，比如“200”。（不需要带px）
//height	弹出层的高	同 width
//drag	是否可拖动	必填，可选参数（true,false）
//time	自动消失时间	可不填，默认不自动关闭；参数可为空("")
//showbg	是否显示遮罩层	可不填，默认不显示（此项如填了，它前面的time也必须要填）
//cssName	弹出层附加样式名	可不填
document.write('<link rel="stylesheet" href="/Img/txpopup.css" />');
$.effect.popup = function (title, content, width, height, drag, time, showbg, cssName) {
    var showWindown = true;
    var templateSrc = "/Img/loading.gif";//设置loading图片路径
    ...
    $.effect.popup.close = closeWindown;
}
