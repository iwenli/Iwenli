    ...
    var initPullToRefreshJS = function (pageContainer) {
        ...
        function handleScroll() {
            ...
            if (scroller.scrollTop() * -1 >= 44) {
                ...
                container.find('.preloader-txt').text('松开刷新...');
            } else {
                container.removeClass('pull-up').addClass('pull-down');
                container.find('.preloader-txt').text('下拉刷新...');
            }
        }
        ...
        function handleRefresh() {
            ...
            container.find('.preloader-txt').text('正在刷新...');
    ...
    $.initPullToRefresh = function (pageContainer) {
        ...
        function handleTouchMove(e) {
            ...
            if (touchesDiff > 0 && scrollTop <= 0 || scrollTop < 0) {
                // iOS 8 fix
                //if ($.device.ios && parseInt($.device.osVersion.split('.')[0], 10) > 7 && scrollTop === 0 && !wasScrolled)
                //{ useTranslate = true; }
                if (useTranslate) {
                    e.preventDefault();
                ...
                if ((useTranslate && Math.pow(touchesDiff, 0.85) > triggerDistance) || (!useTranslate && touchesDiff >= triggerDistance * 2)) {
                    ...
                    container.find('.preloader-txt').text('松开刷新...');
                    //modifi by zhangyulong at 2017年6月9日11:47:40
                    if ($.device.ios && pageY > $(window).height()) {
                        handleTouchEnd();
...
+function ($) {
    ...
    Router.prototype.load = function (url, ignoreCache) {
        ...
        if (this._isTheSameDocument(location.href, url)) {
            this._switchToSection(Util.getUrlFragment(url), DIRECTION.rightToLeft);
        } else {
    ...
    * @param {String} url url
    */
    Router.prototype.backLoad = function (url) {
        if (this._isTheSameDocument(location.href, url)) {
            this._switchToSection(Util.getUrlFragment(url), DIRECTION.leftToRight);
        } else {
            this._saveDocumentIntoCache($(document), location.href);
            this._switchToDocument(url, false, true, DIRECTION.leftToRight);
        }
    };
