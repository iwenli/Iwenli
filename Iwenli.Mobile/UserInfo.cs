...
namespace Iwenli.Mobile
{
    ...
    public class UserInfo
    {
        ...
        public void NoteUserReqInfo(ReqMsg reqMsg)
        {
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    string _jsonStr = JsonHelper.SerializeObject(reqMsg);
                    ...
                    helper.SpGetReturnValue("SP_Service_Iwenli_InsertMessage");
                }
                this.LogInfo("记录用户请求消息成功");
            }
            catch (Exception ex)
            {
                this.LogError("记录用户请求消息错误：" + ex.Message, ex);
            }
        }
        ...
        public void NoteSignUserReqInfo(ReqMsg reqMsg, int isSin, string openid, int ischeck = 1)
        {
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    ...
                    helper.SpGetReturnValue("SP_Service_Iwenli_InsertSignMessage");
                }
                this.LogInfo("记录签到消息成功");
            }
            catch (Exception ex)
            {
                this.LogError("记录签到消息错误：" + ex.Message, ex);
            }
        }
        ...
        /// </summary>
        /// <param name="reqMsg"></param>
        public void NoteUserResMsg(ResMsg reqMsg)
        {
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    ...
                    helper.SpFileValue["@restype"] = reqMsg.ResType;
                    helper.SpGetReturnValue("SP_Service_Iwenli_InsertMessage");
                }
                this.LogInfo("记录向用户发送的消息成功");
            }
            catch (Exception ex)
            {
                this.LogError("记录向用户发送的消息错误：" + ex.Message, ex);
            }
        }
        ...
        public void NoteUserResMsg(ResMsg reqMsg, long messageId)
        {
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    ...
                    helper.SpGetReturnValue("SP_Service_Iwenli_InsertMessageOfReqMsgId");
                }
                this.LogInfo("记录向用户发送的消息成功");
            }
            catch (Exception ex)
        ...
        public long EnSubscribe(ReqEventMsg reqMsg)
        {
            ...
            {
                this.LogInfo("更新usermap信息错误：" + ex.Message);
            }
            return _userMapId;
        }
        ...
        public void UpdateLastScanTime()
        {
            ...
            {
                string sql =
                    "update [dbo].[platform_AccountUserMap] set last_scan_time=getdate() where user_map_id=" +
                    UserMapId;
        ...
        public static UserInfo GetUserInfoByUserMapId(long user_map_id)
        {
            ...
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    string _sql = "SELECT * FROM [dbo].[View_V0_Server_Iwenli_UserInfo] WHERE [user_map_id]=" + user_map_id;
        ...
        public static UserInfo GetUserInfoByOpenId(string openId)
        {
            ...
            try
            {
                using (DataHelper helper = DataHelper.GetDataHelper("IwenliMobile"))
                {
                    string _sql = "SELECT * FROM [dbo].[View_V0_Server_Iwenli_UserInfo] WHERE [openid]='" + openId + "'";
